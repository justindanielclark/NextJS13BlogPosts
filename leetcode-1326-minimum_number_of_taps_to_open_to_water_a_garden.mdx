...
### My first approach
This was something I had to actually at least get a working copy out of before I knew it was incorrect, was to track which points required which taps to be open in order to be covered. I figured that if I knew which idx's in the garden had only 1 possible tap that worked for them, these taps were by default part of the minimum requirements. This resulted in the below cursory implementation:
...
function minTaps(n: number, ranges: number[]): number {
  ...
  while (!error && searching) {
    let [l, c, r] = calculateLeftCenterRight(centerPointer, searchRange, n);
...
At the start, we just declare some tracking variables. We additionally use one O(n) pass on the array to figure out what the strongest possible hose is, as we want to look for it on each iteration if we can:
```ts
let numOps = 0;
let searchRange = 0;
let error = false;
